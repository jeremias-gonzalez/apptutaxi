...
async function calcularRutaDirecta(origen, destino) {
    // Usamos 'driving-traffic' en lugar de driving normal para obtener la ruta m├ís r├ípida considerando el tr├ífico en vivo.
...
// --- BUSCADOR H├ìBRIDO OPTIMIZADO CON MAPBOX GEOCODING ---
async function buscarSugerenciasHibridas(query) {
    ...
    if (!tieneNumero) {
        if (queryLower.includes("peron oeste") || queryLower.includes("per├│n oeste")) {
            busquedaParaApi = busquedaParaApi.replace(/peron oeste|per├│n oeste/i, "Avenida Presidente Per├│n Oeste");
        }
        else if (queryLower.includes("peron este") || queryLower.includes("per├│n este")) {
            busquedaParaApi = busquedaParaApi.replace(/peron este|per├│n este/i, "Avenida Presidente Per├│n Este");
        }
    }
    ...
    try {
        ...
        if (data.features && data.features.length > 0) {
            data.features.forEach(feature => {
                ...
                // Contexto (ej. R├¡o Cuarto, C├│rdoba)
                let ciudad = "R├¡o Cuarto";
                if (feature.context) {
...
function configurarInput(inputElement) {
    ...
    inputElement.addEventListener('input', function () {
        ...
        // Mover la lista din├ímicamente debajo del input que se est├í usando
...
// Ya no usamos resolverUbicacion porque la API de Mapbox nos da las lat/lng directamente en la primera petici├│n. (Se borra la funci├│n)
    ...
    if ("geolocation" in navigator) {
        ...
        const onExito = async function (position) {
            ...
            try {
                ...
                } else { inputOrigen.value = "Ubicaci├│n detectada"; }
            } catch (e) { inputOrigen.value = "Mi Ubicaci├│n Actual"; }
        ...
        navigator.geolocation.getCurrentPosition(onExito, function (error) {
            navigator.geolocation.getCurrentPosition(onExito, (err) => {
                ...
                inputOrigen.placeholder = "Escribe tu direcci├│n...";
...
function pedirPorWhatsapp(tipo) {
    ...
    // Si no est├í definida el origen real
    if (!origen || origen === "Ubicaci├│n detectada" || origen === "Mi Ubicaci├│n Actual") {
        showToast("Por favor selecciona una direcci├│n exacta primero.", "error");
        return;
    }
