...
let realtimeViaje    = null;
let ultimasCoordenadas = { origen: null, destino: null };
let nearbyMarkers = {}; // Almacena marcadores de choferes cercanos
...
async function calcularRutaDirecta(coordenadasViaje) {
    ...
    try {
        ...
        // ── Guardar coordenadas para solicitud de viaje ───────
        ultimasCoordenadas = {
            origen:  { lat: coordenadasViaje[0].lat,                             lng: coordenadasViaje[0].lng },
            destino: { lat: coordenadasViaje[coordenadasViaje.length - 1].lat,   lng: coordenadasViaje[coordenadasViaje.length - 1].lng }
        };
...
async function procesarCalculo() {
    ...
    // Mostrar UI de Carga (Uber Style)
    document.getElementById('panel-inputs').classList.add('hidden');
    document.getElementById('panel-resultados').classList.add('visible');
    document.getElementById('panel-resultados-content').classList.add('hidden');
...
function calcularPrecio() {
    ...
    // Actualizar solo los elementos que existen en el nuevo diseño
    const elFinal = document.getElementById('precio-final');
    if (elFinal) elFinal.innerText = resultadoFinal.toLocaleString('es-AR', { style: 'currency', currency: 'ARS', maximumFractionDigits: 0 });
    
    const elDist = document.getElementById('distancia-final');
    if (elDist) elDist.innerText = km.toFixed(1) + ' km';
    
    const elOriginal = document.getElementById('precio-original');
    if (elOriginal) elOriginal.innerText = calculoBruto.toLocaleString('es-AR', { style: 'currency', currency: 'ARS' });
}
...
// Ride request logic moved to matching.js
window.addEventListener('load', () => {
    cargarLugaresVIP();
    cargarTarifasDB().catch(() => cargarTarifas());
...
function suscribirConductoresCercanos() {
    ...
    db.from('conductores')
        .select('id, lat, lng, disponible, activo')
        .eq('disponible', true)
        .eq('activo', true)
        .then(({ data }) => {
            if (data) data.forEach(actualizarMarcadorNearby);
        });

    // 2. Escuchar cambios en tiempo real
    db.channel('nearby-drivers')
        .on('postgres_changes', 
            { event: '*', schema: 'public', table: 'conductores' }, 
            (payload) => {
                actualizarMarcadorNearby(payload.new || payload.old);
            })
        .subscribe();
}
